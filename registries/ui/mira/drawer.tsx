  ...
  return (
    <AnimatePresence>
      {(props?.isOpen || state?.isOpen) && (
        <DrawerOverlay
          className={cn(
            "data-exiting:fade-out-0 data-entering:fade-in-0 fixed inset-0 z-50 bg-black/80 data-entering:animate-in data-exiting:animate-out supports-backdrop-filter:backdrop-blur-xs"
          )}
          isDismissable
        ...
        >
          {({ state }) => (
            <Modal>
              ...
              >
                {notch && side === "bottom" && (
                  <div className="notch mx-auto h-1.5 w-25 shrink-0 shrink-0 touch-pan-y rounded-full bg-muted group-data-[direction=bottom]/drawer-content:mt-4 group-data-[direction=top]/drawer-content:mb-4" />
                )}
                {children as ReactNode}
                {notch && side === "top" && (
                  <div className="notch mx-auto mx-auto h-1.5 w-25 shrink-0 shrink-0 touch-pan-y rounded-full bg-muted group-data-[direction=bottom]/drawer-content:mt-4 group-data-[direction=top]/drawer-content:mb-4" />
                )}
              </DrawerRoot>
            </Modal>
          )}
...
const DrawerHeader = ({ className, ...props }: ComponentPropsWithRef<"div">) => {
  return (
    <div
      className={cn(
        "flex flex-col gap-1 p-4 group-data-[direction=bottom]/drawer-content:text-center group-data-[direction=top]/drawer-content:text-center md:text-left",
        className
      )}
      slot="header"
      {...props}
...
const DrawerTitle = ({ className, ...props }: RACHeadingProps) => (
  <Heading className={cn("font-medium text-foreground text-sm", className)} slot="title" {...props} />
)
