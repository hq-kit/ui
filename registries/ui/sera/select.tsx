...
const SelectTrigger = ({
  ...
  return context.selectionMode === "multiple" ? (
    <Group
      className={cn(
        "border-transparent border-b-input bg-transparent data-placeholder:text-muted-foreground has-aria-expanded:border-b-ring aria-invalid:border-b-destructive dark:aria-invalid:border-b-destructive/50 gap-1.5 rounded-none border py-2 px-0 text-sm transition-[color,border-color] data-[size=default]:h-10 data-[size=sm]:h-9 *:data-[slot=select-value]:flex *:data-[slot=select-value]:gap-1.5 [&_svg:not([class*='size-'])]:size-3.5 relative flex max-h-none min-h-fit w-fit items-center justify-between whitespace-nowrap py-1 outline-none transition-[color,box-shadow,border] disabled:cursor-not-allowed disabled:opacity-50 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className
      )}
      data-size={size}
    ...
    <Button
      className={cn(
        "border-transparent border-b-input bg-transparent data-placeholder:text-muted-foreground aria-expanded:border-b-ring aria-invalid:border-b-destructive dark:aria-invalid:border-b-destructive/50 gap-1.5 rounded-none border py-2 px-0 text-sm transition-[color,border-color] data-[size=default]:h-10 data-[size=sm]:h-9 *:data-[slot=select-value]:flex *:data-[slot=select-value]:gap-1.5 [&_svg:not([class*='size-'])]:size-3.5 flex w-fit items-center justify-between whitespace-nowrap outline-none transition-[color,box-shadow,border] disabled:cursor-not-allowed disabled:opacity-50 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className
      )}
      data-size={size}
      data-slot="select-trigger"
        ...
        >
          <div className="p-1" data-slot="command-input-wrapper">
            <SearchField
              aria-label="Filter"
              autoFocus
              className={cn(inputGroupVariants({ className: "bg-transparent border-transparent border-b-input px-3" }), className)}
            ...
            >
              ...
              <Input
                className="w-full text-sm px-2 w-auto! outline-hidden disabled:cursor-not-allowed disabled:opacity-50 [&::-webkit-search-cancel-button]:hidden"
...
const SelectGroup = <T extends object>({ title, children, ...props }: ListBoxSectionProps<T> & { title?: string }) => (
  <ListBoxSection className={cn("scroll-my-1.5 p-1.5", props.className)} data-slot="select-group" {...props}>
    {title && (
      <Header className="px-3 py-2 text-xs uppercase tracking-wider font-semibold text-muted-foreground" data-slot="select-label">
        {title}
      </Header>
    )}
...
const SelectItem = ({ className, children, ...props }: ListBoxItemProps) => {
  ...
  return (
    <ListBoxItem
      className={cn(
        "data-focused:bg-accent data-focused:text-accent-foreground not-data-[variant=destructive]:data-focused:**:text-accent-foreground gap-2.5 rounded-none py-2 pr-8 pl-3 text-sm [&_svg:not([class*='size-'])]:size-3.5 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2 relative flex w-full cursor-default select-none items-center outline-hidden data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className
      )}
...
const SelectSeparator = ({ className, ...props }: SeparatorProps) => (
  <Separator
    className={cn("bg-border/50 -mx-1.5 my-1.5 h-px pointer-events-none", className)}
