...
function SheetOverlay({
  ...
  return (
    <ModalOverlayPrimitive
      className={cn(
        "bg-black/10 text-xs/relaxed supports-backdrop-filter:backdrop-blur-xs fixed inset-0 z-50 transition-opacity duration-150 data-entering:opacity-0 data-exiting:opacity-0",
        className
      )}
...
function SheetHeader({ className, ...props }: ComponentProps<"div">) {
  return <div className={cn("gap-0.5 p-4 flex flex-col", className)} data-slot="sheet-header" {...props} />
}
...
function SheetDescription({ className, ...props }: Omit<ComponentProps<"div">, "slot">) {
  return <div className={cn("text-muted-foreground text-xs/relaxed", className)} data-slot="sheet-description" {...props} />
}
