...
const GridListSection = <T extends object>({
  ...
  return (
    <RACGridListSection className={cn("text-foreground **:data-[slot=title]:text-muted-foreground overflow-hidden p-1 **:data-[slot=title]:px-2.5 **:data-[slot=title]:py-1.5 **:data-[slot=title]:text-xs **:data-[slot=title]:font-medium", className)} data-slot="grid-list-section" {...props}>
      {title && (
