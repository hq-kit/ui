    ...
    <div className='w-full max-w-md'>
      <Tabs className='flex-row' defaultSelectedKey='explore'>
        <TabsList className='h-full flex-col rounded-none border-l bg-background p-0'>
          {tabs.map((tab) => (
            <TabsTrigger
              className='size-full justify-start data-selected:*:data-[slot=selected-indicator]:border-primary *:data-[slot=selected-indicator]:left-0 *:data-[slot=selected-indicator]:rounded-none *:data-[slot=selected-indicator]:border-l-2 *:data-[slot=selected-indicator]:bg-background *:data-[slot=selected-indicator]:shadow-none dark:*:data-[slot=selected-indicator]:border-primary'
              id={tab.value}
              key={tab.value}
            >
              {tab.name}
