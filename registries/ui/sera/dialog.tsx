...
const DialogOverlay = ({
  ...
  <ModalOverlay
    className={cn(
      "data-exiting:fade-out-0 data-entering:fade-in-0 fixed inset-0 isolate z-50 bg-black/20 duration-100 data-entering:animate-in data-exiting:animate-out supports-backdrop-filter:backdrop-blur-sm",
      className
    )}
...
const DialogTitle = ({ className, ...props }: Omit<ComponentProps<typeof Heading>, "slot">) => (
  <Heading
    className={cn("font-semibold text-lg uppercase leading-none tracking-wider", className)}
...
const DialogBody = ({ className, ...props }: ComponentPropsWithRef<"div">) => (
  <div
    className={cn(
      "no-scrollbar isolate flex max-h-[calc(var(--visual-viewport-height)-var(--visual-viewport-vertical-padding))] flex-col gap-2 overflow-auto will-change-scroll",
      className
    )}
    data-slot="body"
    {...props}
