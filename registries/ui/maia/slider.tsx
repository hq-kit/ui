...
const Slider = ({ className, children, ...props }: SliderProps) => (
  ...
  >
    {({ state }) => (
      <>
        {children}
        <SliderTrack className="bg-muted rounded-4xl data-[orientation=horizontal]:h-3 data-[orientation=horizontal]:w-full data-[orientation=vertical]:h-full data-[orientation=vertical]:w-3 relative block grow select-none" data-slot="slider-track">
          <SliderFill className="bg-primary select-none rounded-[inherit]" data-slot="slider-range" />
          {state.values.map((_, i) => (
            <SliderThumb
              className="border-primary ring-ring/50 size-4 rounded-4xl border bg-white shadow-sm transition-colors hover:ring-4 focus-visible:ring-4 focus-visible:outline-hidden select-none disabled:pointer-events-none disabled:opacity-50 group-data-[orientation=horizontal]/slider:top-1/2 group-data-[orientation=vertical]/slider:left-1/2"
