...
const Command = ({
  ...
  <Autocomplete data-slot="command" {...props}>
    <div
      className={cn("bg-popover text-popover-foreground rounded-none group/command flex size-full flex-col overflow-hidden", className)}
...
const CommandInput = ({
  ...
  <div className="border-b pb-0" data-slot="command-input-wrapper">
    ...
    >
      ...
      <Input
        className="w-full text-xs outline-hidden disabled:cursor-not-allowed disabled:opacity-50 [&::-webkit-search-cancel-button]:hidden"
...
const CommandList = <T extends object>({ className, ...props }: MenuProps<T>) => (
  <Menu
    className={cn(
      "no-scrollbar max-h-72 scroll-py-0 outline-none scroll-fade-y overflow-y-auto overflow-x-hidden p-1 has-data-[slot=command-group]:p-0",
      className
    )}
...
const CommandGroup = <T extends object>({ className, title, ...props }: MenuSectionProps<T> & { title?: string }) => (
  <MenuSection className={cn("text-foreground **:data-[slot=title]:text-muted-foreground overflow-hidden **:data-[slot=title]:px-2 **:data-[slot=title]:py-1.5 **:data-[slot=title]:text-xs", className)} data-slot="command-group">
    {title && (
...
const CommandItem = ({ className, ...props }: MenuItemProps) => (
  <MenuItem
    className={cn(
      "data-hovered:bg-muted data-hovered:text-foreground data-hovered:*:[svg]:text-foreground data-focused:bg-muted data-focused:text-foreground data-focused:*:[svg]:text-foreground relative flex cursor-default items-center gap-2 rounded-none px-2 py-2 text-xs outline-hidden select-none in-data-[slot=dialog-content]:rounded-none! [&_svg:not([class*='size-'])]:size-4 group/command-item data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
      className
    )}
