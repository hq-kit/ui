  ...
  return (
    <AnimatePresence>
      {(props?.isOpen || state?.isOpen) && (
        <DrawerOverlay
          className={cn(
            "data-exiting:fade-out-0 data-entering:fade-in-0 fixed inset-0 z-50 bg-black/30 data-entering:animate-in data-exiting:animate-out supports-backdrop-filter:backdrop-blur-sm"
          )}
          isDismissable
...
const DrawerTitle = ({ className, ...props }: RACHeadingProps) => (
  <Heading className={cn("font-medium text-base text-foreground", className)} slot="title" {...props} />
)
