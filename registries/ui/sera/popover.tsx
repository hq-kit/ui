...
function PopoverTitle({ className, ...props }: ComponentProps<typeof Heading>) {
  return <Heading className={cn("text-xs font-semibold uppercase", className)} data-slot="popover-title" {...props} />
}

function PopoverDescription({ className, ...props }: ComponentProps<"div">) {
  return <div className={cn("text-muted-foreground text-sm leading-relaxed mt-0.5", className)} data-slot="popover-description" {...props} />
}
