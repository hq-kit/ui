...
const AlertDialogContent = ({
  ...
  <AlertDialogOverlay className={overlayClassName} {...props}>
    <Modal
      className={composeRenderProps(className, (className) =>
        cn(
          "bg-popover text-popover-foreground data-entering:animate-in data-exiting:animate-out data-exiting:fade-out-0 data-entering:fade-in-0 data-exiting:zoom-out-95 data-entering:zoom-in-95 ring-foreground/5 dark:ring-foreground/10 max-w-[calc(100%-2rem)] rounded-[min(var(--radius-4xl),24px)] shadow-xl ring-1 duration-100 sm:max-w-md group/alert-dialog-content fixed top-1/2 left-1/2 z-50 w-full -translate-x-1/2 -translate-y-1/2 outline-hidden",
          className
        )
      )}
      data-size={size}
