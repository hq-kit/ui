...
const SelectTrigger = ({
  ...
  return context.selectionMode === "multiple" ? (
    <Group
      className={cn(
        "border-input data-placeholder:text-muted-foreground bg-input/20 dark:bg-input/30 dark:hover:bg-input/50 has-aria-expanded:border-ring has-aria-expanded:ring-ring/30 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive dark:aria-invalid:border-destructive/50 gap-1.5 rounded-md border px-2 py-1.5 text-xs/relaxed transition-colors has-aria-expanded:ring-2 aria-invalid:ring-2 data-[size=default]:h-7 data-[size=sm]:h-6 *:data-[slot=select-value]:flex *:data-[slot=select-value]:gap-1.5 [&_svg:not([class*='size-'])]:size-3.5 relative flex max-h-none min-h-fit w-fit items-center justify-between whitespace-nowrap py-1 outline-none transition-[color,box-shadow,border] disabled:cursor-not-allowed disabled:opacity-50 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className
      )}
      data-size={size}
    ...
    <Button
      className={cn(
        "border-input data-placeholder:text-muted-foreground bg-input/20 dark:bg-input/30 dark:hover:bg-input/50 aria-expanded:border-ring aria-expanded:ring-ring/30 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive dark:aria-invalid:border-destructive/50 gap-1.5 rounded-md border px-2 py-1.5 text-xs/relaxed transition-colors aria-expanded:ring-2 aria-invalid:ring-2 data-[size=default]:h-7 data-[size=sm]:h-6 *:data-[slot=select-value]:flex *:data-[slot=select-value]:gap-1.5 [&_svg:not([class*='size-'])]:size-3.5 flex w-fit items-center justify-between whitespace-nowrap outline-none transition-[color,box-shadow,border] disabled:cursor-not-allowed disabled:opacity-50 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className
      )}
      data-size={size}
      data-slot="select-trigger"
        ...
        >
          <div className="p-1 pb-0" data-slot="command-input-wrapper">
            <SearchField
              aria-label="Filter"
              autoFocus
              className={cn(inputGroupVariants({ className: "bg-input/20 dark:bg-input/30 h-8!" }), className)}
              data-slot="command-input-wrapper"
            >
              <InputGroupAddon>
                <IconPlaceholder
                  className="size-3.5 shrink-0 opacity-50 group-data-[pending=true]/command:hidden"
                ...
                <IconPlaceholder
                  aria-label="Loading"
                  className="size-3.5 shrink-0 opacity-50 hidden animate-spin group-data-[pending=true]/command:block"
                  data-slot="input-group-addon"
              ...
              <Input
                className="w-full text-xs/relaxed w-auto! outline-hidden disabled:cursor-not-allowed disabled:opacity-50 [&::-webkit-search-cancel-button]:hidden"
...
const SelectGroup = <T extends object>({ title, children, ...props }: ListBoxSectionProps<T> & { title?: string }) => (
  <ListBoxSection className={cn("scroll-my-1 p-1", props.className)} data-slot="select-group" {...props}>
    {title && (
      <Header className="text-muted-foreground px-2 py-1.5 text-xs" data-slot="select-label">
        {title}
      </Header>
    )}
...
const SelectItem = ({ className, children, ...props }: ListBoxItemProps) => {
  ...
  return (
    <ListBoxItem
      className={cn(
        "data-focused:bg-accent data-focused:text-accent-foreground not-data-[variant=destructive]:data-focused:**:text-accent-foreground min-h-7 gap-2 rounded-md px-2 py-1 text-xs/relaxed [&_svg:not([class*='size-'])]:size-3.5 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2 relative flex w-full cursor-default select-none items-center outline-hidden data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className
      )}
