...
const ListBox = <T extends object>({ className, ...props }: ListBoxProps<T>) => (
  <RACListBox
    ...
    className={composeRenderProps(className, (className) =>
      cn(
        "group/list-box flex size-full flex-col gap-1 overflow-hidden rounded-3xl border bg-popover p-2 text-popover-foreground [--radius:var(--radius-xl)]",
...
const ListBoxItem = ({ children, className, ...props }: ListBoxItemProps) => {
  ...
  return (
    <RACListBoxItem
      className={composeRenderProps(className, (className) =>
        cn(
          "group/list-box-item relative flex min-h-7 cursor-default select-none items-center gap-2 in-data-[slot=dialog-content]:rounded-2xl rounded-xl px-2 py-1.5 text-sm outline-hidden data-[disabled=true]:pointer-events-none data-selected:bg-muted data-selected:text-foreground data-[disabled=true]:opacity-50 [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0 data-selected:*:[svg]:text-foreground",
