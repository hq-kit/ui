...
const Command = ({
  ...
  <Autocomplete data-slot="command" {...props}>
    <div
      className={cn("bg-popover text-popover-foreground rounded-3xl p-1 group/command flex size-full flex-col overflow-hidden", className)}
...
const CommandDialog = ({
  ...
  <Dialog {...props}>
    ...
    <Dialog.Content
      className={cn("rounded-3xl! p-0 top-1/3 translate-y-0 overflow-hidden *:p-0", className)}
...
const CommandItem = ({ className, ...props }: MenuItemProps) => (
  <MenuItem
    className={cn(
      "data-hovered:bg-muted data-hovered:text-foreground data-hovered:*:[svg]:text-foreground data-focused:bg-muted data-focused:text-foreground data-focused:*:[svg]:text-foreground relative flex cursor-default items-center gap-2 min-h-7 rounded-xl px-2 py-1.5 text-sm outline-hidden select-none in-data-[slot=dialog-content]:rounded-2xl [&_svg:not([class*='size-'])]:size-4 group/command-item data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
      className
    )}
