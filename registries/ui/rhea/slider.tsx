...
const Slider = ({ className, children, ...props }: SliderProps) => (
  ...
  >
    {({ state }) => (
      <>
        {children}
        <SliderTrack className="bg-input/90 rounded-2xl data-[orientation=horizontal]:h-1 data-[orientation=horizontal]:w-full data-[orientation=vertical]:h-full data-[orientation=vertical]:w-1 relative block grow select-none" data-slot="slider-track">
          <SliderFill className="bg-primary select-none rounded-[inherit]" data-slot="slider-range" />
          {state.values.map((_, i) => (
            <SliderThumb
              className="ring-black/10 not-dark:bg-clip-padding ring-1 size-4 rounded-2xl bg-white shadow-md transition-[color,box-shadow] duration-200 hover:ring-4 hover:ring-ring/30 focus-visible:ring-4 focus-visible:ring-ring/30 focus-visible:outline-hidden select-none disabled:pointer-events-none disabled:opacity-50 group-data-[orientation=horizontal]/slider:top-1/2 group-data-[orientation=vertical]/slider:left-1/2"
