...
const GridListSection = <T extends object>({
  ...
  return (
    <RACGridListSection className={cn("text-foreground **:data-[slot=title]:text-muted-foreground overflow-hidden **:data-[slot=title]:px-2 **:data-[slot=title]:py-1.5 **:data-[slot=title]:text-xs", className)} data-slot="grid-list-section" {...props}>
      {title && (
