...
Sometimes, you may want to add components individually, such as `select` and `input`. You can easily do this by using the following command:
<CLI command='add' items={['select', 'input']} />
...
```tsx title='components/ui/index.ts'
export * from './avatar'
export * from './badge'
