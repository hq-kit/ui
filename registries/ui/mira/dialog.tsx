...
const DialogOverlay = ({ className, isDismissable = true, ...props }: ModalOverlayProps) => (
  <RACModalOverlay
    className={composeRenderProps(className, (className) =>
      cn("data-entering:animate-in data-exiting:animate-out data-exiting:fade-out-0 data-entering:fade-in-0 bg-black/80 duration-100 supports-backdrop-filter:backdrop-blur-xs fixed inset-0 isolate z-50", className)
    )}
    data-slot="overlay"
  ...
  return (
    <DialogOverlay className={overlayClassName} isDismissable={isDismissable} {...props}>
      ...
      >
        <RACDialog
          aria-label="Dialog"
          className="grid gap-4 p-4 text-xs/relaxed relative size-full outline-hidden"
...
const DialogHeader = ({ className, ...props }: ComponentPropsWithRef<"div">) => (
  <div className={cn("gap-1 flex flex-col", className)} data-slot="header" {...props} />
)

const DialogTitle = ({ className, ...props }: RACHeadingProps) => (
  <Heading className={cn("text-sm font-medium", className)} data-slot="title" {...props} />
)

const DialogDescription = ({ className, ...props }: TextProps) => (
  <Text className={cn("text-muted-foreground *:[a]:hover:text-foreground text-xs/relaxed *:[a]:underline *:[a]:underline-offset-3", className)} data-slot="description" {...props} />
)
...
const DialogX = (props: ButtonProps) => (
  <Button aria-label="Close" className="absolute top-2 right-2" size="icon-sm" slot="close" variant="ghost" {...props}>
