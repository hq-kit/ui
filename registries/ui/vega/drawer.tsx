  ...
  return (
    <AnimatePresence>
      {(props?.isOpen || state?.isOpen) && (
        <DrawerOverlay
          className={cn("data-entering:animate-in data-exiting:animate-out data-exiting:fade-out-0 data-entering:fade-in-0 bg-black/10 supports-backdrop-filter:backdrop-blur-xs fixed inset-0 z-50")}
          isDismissable
          ...
          {({ state }) => (
            <Modal>
              <DrawerRoot
                animate={{ x: 0, y: 0 }}
                aria-label="Drawer"
                className={cn("group/drawer-content bg-popover text-popover-foreground flex h-auto flex-col text-sm data-[direction=bottom]:inset-x-0 data-[direction=bottom]:bottom-0 data-[direction=bottom]:mt-24 data-[direction=bottom]:max-h-[80vh] data-[direction=bottom]:rounded-t-xl data-[direction=bottom]:border-t data-[direction=left]:inset-y-0 data-[direction=left]:left-0 data-[direction=left]:w-3/4 data-[direction=left]:rounded-r-xl data-[direction=left]:border-r data-[direction=right]:inset-y-0 data-[direction=right]:right-0 data-[direction=right]:w-3/4 data-[direction=right]:rounded-l-xl data-[direction=right]:border-l data-[direction=top]:inset-x-0 data-[direction=top]:top-0 data-[direction=top]:mb-24 data-[direction=top]:max-h-[80vh] data-[direction=top]:rounded-b-xl data-[direction=top]:border-b data-[direction=left]:sm:max-w-sm data-[direction=right]:sm:max-w-sm fixed z-50 touch-none", className)}
                data-direction={side}
              ...
              >
                {notch && side === "bottom" && <div className="notch bg-muted mx-auto group-data-[direction=bottom]/drawer-content:mt-4 group-data-[direction=top]/drawer-content:mb-4 hidden h-1.5 w-25 shrink-0 rounded-full shrink-0 touch-pan-y" />}
                {children as ReactNode}
                {notch && side === "top" && <div className="notch bg-muted mx-auto group-data-[direction=bottom]/drawer-content:mt-4 group-data-[direction=top]/drawer-content:mb-4 hidden h-1.5 w-25 shrink-0 rounded-full mx-auto shrink-0 touch-pan-y" />}
              </DrawerRoot>
            </Modal>
          )}
        </DrawerOverlay>
...
const DrawerHeader = ({ className, ...props }: ComponentPropsWithRef<"div">) => {
  return <div className={cn("gap-0.5 p-4 group-data-[direction=bottom]/drawer-content:text-center group-data-[direction=top]/drawer-content:text-center md:gap-1.5 md:text-left flex flex-col", className)} slot="header" {...props} />
}

const DrawerTitle = ({ className, ...props }: RACHeadingProps) => (
  <Heading className={cn("text-foreground font-medium", className)} slot="title" {...props} />
)
