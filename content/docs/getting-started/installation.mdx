...
@layer base {
    ...
    .dark {
        --bg: hsl(240 10% 3.92%);
        --fg: hsl(0 0% 98.04%);
        --primary: hsl(0 0% 98.04%);
        --primary-fg: hsl(240 10% 3.92%);
        --secondary: hsl(208.24 7.36% 54.71%);
        --secondary-fg: hsl(0 0% 100%);
        --primary: hsl(240, 5.26%, 26.08%);
        --primary-fg: hsl(0 0% 98.04%);
...
<Code  lang='ts' code={`import { clsx, type ClassValue } from "clsx";
...
### useMediaQuery Hooks
Create a hook called `useMediaQuery` (some components which Responsive are need it). Usually located in `hooks/use-media-query.tsx`:
