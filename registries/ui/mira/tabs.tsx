...
const tabsListVariants = tv({
  base: "rounded-lg p-0.75 group-data-[orientation=horizontal]/tabs:h-8 data-[variant=line]:rounded-none group/tabs-list inline-flex w-fit items-center justify-center text-muted-foreground group-data-[orientation=vertical]/tabs:h-fit group-data-[orientation=vertical]/tabs:flex-col",
  variants: {
    variant: {
      default: "bg-muted",
...
const TabsContents = <T extends object>({ className, ...props }: TabPanelsProps<T>) => (
  <RACTabPanels
    {...props}
    className={cn(
      "text-xs/relaxed relative h-(--tab-panel-height) flex-1 outline-none motion-safe:transition-[height]",
      className
    )}
  />
)
