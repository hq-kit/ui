...
const AccordionSplitDemo = () => {
  return (
    <Accordion className='w-full space-y-2' defaultExpandedKeys='item-1'>
      {items.map((item, index) => (
        <AccordionItem
          className='rounded-md border-b-0 bg-card shadow-md data-[expanded=true]:shadow-lg'
          id={`item-${index + 1}`}
          key={index}
        >
          <AccordionTrigger className='px-5 [&>svg]:rotate-90 in-data-[expanded=true]:*:[svg]:rotate-0'>
            {item.title}
          </AccordionTrigger>
          <AccordionContent className='px-5 text-muted-foreground'>{item.content}</AccordionContent>
        </AccordionItem>
      ))}
    </Accordion>
  )
