...
const ColorArea = ({ className, ...props }: ColorAreaProps) => (
  <RACColorArea
    className={composeRenderProps(className, (className) =>
      cn(
        "size-48 shrink-0 rounded-none bg-muted shadow-md data-disabled:opacity-50 data-disabled:grayscale-50",
        className
      )
    )}
    {...props}
...
const ColorThumb = ({ className, ...props }: ColorThumbProps) => (
  <RACColorThumb
    className={composeRenderProps(className, (className) =>
      cn(
        "relative size-3 select-none rounded-none border border-ring bg-white ring-ring/50 transition-[color,box-shadow] [box-shadow:0_0_0_1px_var(--background),inset_0_0_0_1px_var(--foreground)] after:absolute after:-inset-2 hover:ring-1 focus-visible:outline-hidden focus-visible:ring-1 active:ring-1 disabled:opacity-50 group-data-[orientation=horizontal]/slider:top-1/2 group-data-[orientation=vertical]/slider:left-1/2",
...
const ColorSwatchPickerItem = ({ className, children, ...props }: ColorSwatchPickerItemProps) => (
  <RACColorSwatchPickerItem
    className={composeRenderProps(className, (className) =>
      cn(
        "relative rounded-none bg-muted outline-hidden",
...
const ColorSwatch = ({ className, ...props }: ColorSwatchProps) => (
  <RACColorSwatch
    className={composeRenderProps(className, (className) =>
      cn(
        "inset-ring-1 inset-ring-current/20 size-6 shrink-0 rounded-none rounded-none bg-muted [&_svg:not([class*='size-'])]:size-3",
        className
      )
    )}
