  ...
  return (
    <Accordion className='w-full space-y-2' defaultExpandedKeys='item-1'>
      {items.map((item, index) => (
        <AccordionItem className='border! rounded-md' id={`item-${index + 1}`} key={index}>
          <AccordionTrigger className='bg-accent px-5 data-[expanded=true]:rounded-b-none'>
            {item.title}
          </AccordionTrigger>
          <AccordionContent className='px-5 pt-4 text-muted-foreground'>{item.content}</AccordionContent>
        </AccordionItem>
      ))}
    </Accordion>
  )
