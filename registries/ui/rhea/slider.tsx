  ...
  >
    {({ state }) => (
      <>
        {children}
        <SliderTrack
          className="relative block grow select-none rounded-2xl bg-input/90 data-[orientation=horizontal]:h-1 data-[orientation=vertical]:h-full data-[orientation=horizontal]:w-full data-[orientation=vertical]:w-1"
          data-slot="slider-track"
        >
          <SliderFill className="select-none rounded-[inherit] bg-primary" data-slot="slider-range" />
          {state.values.map((_, i) => (
            <SliderThumb
              className="size-4 select-none rounded-2xl bg-white not-dark:bg-clip-padding shadow-md ring-1 ring-black/10 transition-[color,box-shadow] duration-200 hover:ring-4 hover:ring-ring/30 focus-visible:outline-hidden focus-visible:ring-4 focus-visible:ring-ring/30 disabled:pointer-events-none disabled:opacity-50 group-data-[orientation=horizontal]/slider:top-1/2 group-data-[orientation=vertical]/slider:left-1/2"
