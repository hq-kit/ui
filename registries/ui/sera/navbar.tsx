...
const Navbar = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "fixed inset-x-0 z-30 mx-auto h-(--navbar-height) bg-sidebar transition-all group-data-[variant=floating]:rounded-none group-data-[variant=floating]:shadow-sm group-data-[variant=floating]:ring-1 group-data-[variant=floating]:ring-sidebar-border",
      ...
      >
        ...
        >
          {isMobile && (
            <div className="mr-2 flex items-center gap-2 p-2 [--radius:0]">
              <NavbarTrigger />
...
const NavbarHeader = ({ className, ...props }: ComponentProps<"div">) => (
  <div
    className={cn("sticky left-0 z-10 flex items-center gap-2 bg-sidebar p-2 [--radius:0]", className)}
    data-navbar="header"
...
const NavbarActions = ({ className, ...props }: ComponentProps<"div">) => (
  <div
    className={cn("ml-auto flex items-center gap-2 p-2 [--radius:0]", className)}
    data-navbar="actions"
...
const NavbarContent = ({ className, ...props }: ComponentProps<"div">) => {
  ...
  return (
    <div
      className={cn(
        "no-scrollbar flex min-h-0 flex-1 gap-2 overflow-auto [--radius:0]",
...
const navbarMenuButtonVariants = tv({
  base: "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-none px-3 py-2 text-left text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&_svg]:size-4 [&_svg]:shrink-0",
  variants: {
    variant: {
