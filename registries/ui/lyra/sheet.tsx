...
const SheetOverlay = ({ className, isDismissable = true, ...props }: ModalOverlayProps) => (
  <ModalOverlay
    className={composeRenderProps(className, (className) =>
      cn(
        "bg-black/10 text-xs/relaxed supports-backdrop-filter:backdrop-blur-xs data-entering:fade-in-0 data-exiting:fade-out-0 fixed inset-0 z-50 duration-200 data-entering:animate-in data-exiting:animate-out",
        className
      )
    )}
    data-slot="overlay"
...
const SheetHeader = ({ className, ...props }: ComponentPropsWithRef<"div">) => (
  <div className={cn("gap-0.5 p-4 flex flex-col", className)} data-slot="sheet-header" {...props} />
)
...
const SheetDescription = ({ className, ...props }: TextProps) => (
  <Text className={cn("text-muted-foreground text-xs/relaxed", className)} data-slot="sheet-description" slot="description" {...props} />
)
...
const SheetX = (props: ButtonProps) => (
  <Button aria-label="Close" className="absolute top-3 right-3" size="icon-sm" slot="close" variant="ghost" {...props}>
