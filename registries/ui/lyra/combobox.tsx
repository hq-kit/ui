...
const ComboboxChips = <T extends object>(props: TagListProps<T> & { isDisabled?: boolean }) => {
  ...
  return (
    <Group
      className="flex min-h-8 flex-wrap items-center gap-1 rounded-none border border-input bg-transparent bg-clip-padding py-1 pr-1.5 pl-2.5 text-xs transition-colors focus-within:border-ring focus-within:ring-1 focus-within:ring-ring/50 has-aria-invalid:border-destructive has-data-[slot=combobox-chip]:px-1 has-aria-invalid:ring-1 has-aria-invalid:ring-destructive/20 dark:bg-input/30 dark:has-aria-invalid:border-destructive/50 dark:has-aria-invalid:ring-destructive/40"
...
const ComboboxContent = <T extends object>({
  ...
  >
    <ListBox
      className="no-scrollbar flex max-h-[calc(var(--visual-viewport-height)-10rem)] max-h-[min(calc(--spacing(72)---spacing(9)),calc(var(--available-height)---spacing(9)))] scroll-py-1 flex-col overflow-y-auto overflow-y-auto overscroll-contain rounded-lg p-1 outline-hidden data-empty:p-0 sm:max-h-[inherit]"
...
const ComboboxItem = <T extends object>({ className, children, ...props }: ListBoxItemProps<T>) => {
  ...
  return (
    <ListBoxItem
      className={cn(
        "relative flex w-full cursor-default select-none items-center gap-2 rounded-none py-2 pr-8 pl-2 text-xs outline-hidden data-disabled:pointer-events-none data-focused:bg-accent data-highlighted:bg-accent data-focused:text-accent-foreground data-highlighted:text-accent-foreground data-disabled:opacity-50 not-data-[variant=destructive]:data-focused:**:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className
      )}
...
const ComboboxGroup = <T extends object>({ title, ...props }: ListBoxSectionProps<T> & { title?: string }) => (
  <ListBoxSection className={cn(props.className)} data-slot="combobox-group" {...props}>
    {title && (
      <Header className="px-2 py-2 text-muted-foreground text-xs" data-slot="combobox-label">
        {title}
      </Header>
    )}
...
const ComboboxSeparator = ({ className, ...props }: SeparatorProps) => (
  <Separator className={cn("-mx-1 h-px bg-border", className)} data-slot="combobox-separator" {...props} />
)
