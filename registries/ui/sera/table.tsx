...
const TableColumn = ({ isResizable = false, className, ...props }: ColumnProps & { isResizable?: boolean }) => {
  return (
    <Column
      ...
      className={composeRenderProps(className, (className) =>
        cn(
          "text-muted-foreground h-12 px-3 text-left align-middle text-xs font-medium uppercase tracking-wider whitespace-nowrap has-[slot=selection]:pr-0 relative",
...
const TableCell = ({ className, ref, ...props }: TableCellProps) => {
  ...
  return (
    <Cell
      {...props}
      className={composeRenderProps(className, (className) =>
        cn("p-3 align-middle whitespace-nowrap has-[slot=selection]:pr-0 outline outline-transparent", allowResize && "overflow-hidden truncate", className)
      )}
