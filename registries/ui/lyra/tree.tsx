...
const TreeItem = <T extends object>({ className, ...props }: TreeItemProps<T>) => (
  <RACTreeItem
    className={composeRenderProps(className, (className) =>
      cn(
        "hover:bg-muted aria-expanded:bg-muted rounded-none px-1.5 py-[calc(--spacing(0.8))] font-medium relative flex cursor-default items-center gap-1 border border-transparent pr-2 text-sm outline-hidden transition",
