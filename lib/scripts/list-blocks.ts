      ...
      if (folderFile === 'page.tsx') {
        ...
        for (const subFolderFile of filesInSubFolder) {
          if (subFolderFile === 'page.tsx') {
            ...
            const filesInSubSubFolder = fs.readdirSync(path.join(blocksDir, file, folderFile, subFolderFile))
    ...
    new Set(
      (content.match(regex) || []).flatMap((match: string) =>
        match
          ?.match(/{([^}]+)}/)?.[1]
          .split(',')
          .map((comp: string) => `${slugify(comp.trim()).replaceAll('-style', '').replaceAll('-content', '')}.tsx`)
      )
    )
  )
