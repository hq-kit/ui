    ...
    <Card>
      ...
      <CardContent className="flex flex-col gap-3">
        <ChartContainer className="h-32 w-full" config={sleepChartConfig}>
          ...
          >
            <Bar dataKey="deep" fill="var(--color-deep)" radius={0} stackId="a" />
            <Bar dataKey="light" fill="var(--color-light)" radius={0} stackId="a" />
            <Bar dataKey="rem" fill="var(--color-rem)" radius={[2, 2, 0, 0]} stackId="a" />
          </BarChart>
        </ChartContainer>
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            <div className="text-center" key={s.label}>
              <div className="font-medium text-sm tabular-nums">{s.value}</div>
