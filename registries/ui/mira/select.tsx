...
const SelectTrigger = ({
  ...
  <Button
    className={cn(
      "flex w-full items-center justify-between gap-1.5 whitespace-nowrap rounded-md border border-input bg-input/20 px-2 py-1.5 text-xs/relaxed outline-none transition-colors focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/30 disabled:cursor-not-allowed disabled:opacity-50 data-[size=default]:h-7 data-[size=sm]:h-6 data-placeholder:text-muted-foreground *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5 group-data-invalid/field:border-destructive group-data-invalid/field:ring-2 group-data-invalid/field:ring-destructive/20 dark:bg-input/30 dark:group-data-invalid/field:border-destructive/50 dark:group-data-invalid/field:ring-destructive/40 dark:hover:bg-input/50 [&_svg:not([class*='size-'])]:size-3.5 [&_svg]:pointer-events-none [&_svg]:shrink-0",
      className
    )}
    data-size={size}
...
const SelectGroup = <T extends object>({ title, children, ...props }: ListBoxSectionProps<T> & { title?: string }) => (
  <ListBoxSection className={cn("scroll-my-1 p-1", props.className)} data-slot="select-group" {...props}>
    {title && (
      <Header className="px-2 py-1.5 text-muted-foreground text-xs" data-slot="select-label">
        {title}
      </Header>
    )}
...
const SelectEmpty = ({ className, ...props }: ComponentProps<"div">) => (
  <div
    className={cn(
      "hidden w-full justify-center py-2 text-center text-muted-foreground text-xs/relaxed group-data-empty/select-list:flex",
      className
    )}
