...
const ColorArea = ({ className, ...props }: ColorAreaProps) => (
  <RACColorArea
    className={composeRenderProps(className, (className) =>
      cn("size-48 shrink-0 bg-muted shadow-md data-disabled:opacity-50 data-disabled:grayscale-50", className)
    )}
    {...props}
  >
    <ColorThumb />
...
const ColorThumb = ({ className, ...props }: ColorThumbProps) => (
  <RACColorThumb
    className={composeRenderProps(className, (className) =>
      cn(
        "size-3 select-none border-none bg-primary transition-colors [box-shadow:0_0_0_1px_var(--background),inset_0_0_0_1px_var(--foreground)] hover:ring-2 hover:ring-ring/30 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring/30 disabled:opacity-50 group-data-[orientation=horizontal]/slider:top-1/2 group-data-[orientation=vertical]/slider:left-1/2",
...
const ColorSwatchPickerItem = ({ className, children, ...props }: ColorSwatchPickerItemProps) => (
  <RACColorSwatchPickerItem
    className={composeRenderProps(className, (className) =>
      cn(
        "relative bg-muted outline-hidden",
...
const ColorSwatch = ({ className, ...props }: ColorSwatchProps) => (
  <RACColorSwatch
    className={composeRenderProps(className, (className) =>
      cn("inset-ring-1 inset-ring-current/20 size-7 shrink-0 bg-muted [&_svg:not([class*='size-'])]:size-3", className)
    )}
