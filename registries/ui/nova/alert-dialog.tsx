...
const AlertDialogContent = ({
  ...
  <AlertDialogOverlay className={overlayClassName} {...props}>
    <Modal
      className={composeRenderProps(className, (className) =>
        cn(
          "rounded-xl duration-100 ring-1 ring-foreground/10 max-w-[calc(100%-2rem)] sm:max-w-sm bg-popover text-popover-foreground data-entering:animate-in data-exiting:animate-out data-exiting:fade-out-0 data-entering:fade-in-0 data-exiting:zoom-out-95 data-entering:zoom-in-95 group/alert-dialog-content fixed top-1/2 left-1/2 z-50 w-full -translate-x-1/2 -translate-y-1/2 outline-hidden",
          className
        )
      )}
      data-size={size}
    ...
    >
      <RACDialog
        ...
        className="grid gap-4 p-4 text-sm relative grid w-full max-w-[inherit] outline-hidden sm:max-w-[inherit]"
...
const AlertDialogMedia = ({ className, ...props }: ComponentProps<"div">) => (
  <div className={cn("bg-muted mb-2 inline-flex size-10 items-center justify-center rounded-md sm:group-data-[size=default]/alert-dialog-content:row-span-2 *:[svg:not([class*='size-'])]:size-6", className)} data-slot="alert-dialog-media" {...props} />
)
...
const AlertDialogDescription = ({ className, ...props }: TextProps) => (
  <Text className={cn("text-muted-foreground *:[a]:hover:text-foreground text-sm text-balance md:text-pretty *:[a]:underline *:[a]:underline-offset-3", className)} data-slot="alert-dialog-description" {...props} />
)
