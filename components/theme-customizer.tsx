      ...
      <Sheet.Content className='sm:max-w-md' side='right'>
        ...
        <Sheet.Body className='space-y-4'>
          <div className='flex w-full flex-col gap-4 sm:flex-row'>
            <Select
              ...
              onSelectionChange={(key) =>
                updateFontSansFamily(fontSansFamilies.find((f) => f.label === (key as string)) ?? fontSansFamilies[2])
              }
            ...
            <Select
              ...
              name='font-mono'
              onSelectionChange={(key) =>
                updateFontMonoFamily(fontMonoFamilies.find((f) => f.label === (key as string)) ?? fontMonoFamilies[2])
              }
              placeholder='Select a font'
          ...
          <div className='group flex flex-col gap-1.5'>
            <div className='font-medium text-muted-foreground text-sm group-has-focus:text-primary'>Preset Theme</div>
            <GridList
              ...
              {(item) => (
                <GridListItem
                  ...
                  style={{
                    ...
                    color: mode === 'dark' ? item?.dark?.['primary-foreground'] : item?.light?.['primary-foreground']
                  }}
          ...
          <div className='grid grid-cols-3 gap-0 *:rounded-none *:first:rounded-l-lg *:last:rounded-r-lg'>
            <Button onPress={() => setMode('light')} size='sm' variant={mode === 'light' ? 'default' : 'outline'}>
              <IconSun />
              Light
            </Button>
            <Button onPress={() => setMode('system')} size='sm' variant={mode === 'system' ? 'default' : 'outline'}>
              <IconDeviceDesktop />
              System
            </Button>
            <Button onPress={() => setMode('dark')} size='sm' variant={mode === 'dark' ? 'default' : 'outline'}>
              <IconMoon />
              Dark
            </Button>
          </div>
