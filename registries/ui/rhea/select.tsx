...
const SelectTrigger = ({
  ...
  <Button
    className={cn(
      "flex w-full items-center justify-between gap-1.5 whitespace-nowrap rounded-2xl border border-transparent bg-input/50 px-3 py-2 text-sm outline-none transition-[color,box-shadow] duration-200 focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/30 disabled:cursor-not-allowed disabled:opacity-50 data-[size=default]:h-8 data-[size=sm]:h-7 data-placeholder:text-muted-foreground *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5 group-data-invalid/field:border-destructive group-data-invalid/field:ring-3 group-data-invalid/field:ring-destructive/20 dark:group-data-invalid/field:border-destructive/50 dark:group-data-invalid/field:ring-destructive/40 [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
      className
    )}
    data-size={size}
...
const SelectGroup = <T extends object>({ title, children, ...props }: ListBoxSectionProps<T> & { title?: string }) => (
  <ListBoxSection className={cn("scroll-my-1.5 p-1", props.className)} data-slot="select-group" {...props}>
    {title && (
      <Header className="px-2 py-1 text-muted-foreground text-xs" data-slot="select-label">
        {title}
      </Header>
    )}
