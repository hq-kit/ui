...
const CopyButton = ({ copied, className, onPress }: { copied: boolean; className?: string; onPress?: () => void }) => (
  <Button
    ...
    variant='ghost'
  >
    <IconCopy className={cn('size-4 rotate-0 scale-100 transition-all duration-200', copied && 'rotate-90 scale-0')} />
    <IconCheck
      className={cn('absolute size-4 rotate-90 scale-0 transition-all duration-200', copied && 'rotate-0 scale-100')}
    />
  </Button>
)
