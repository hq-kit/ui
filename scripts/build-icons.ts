...
function scanIconUsage() {
  ...
  const registryBasesDir = path.join(process.cwd(), "components")
...
function generateIconFiles(iconUsage: IconUsage) {
  const outputDir = path.join(process.cwd(), "lib/icons")
...
`
  ...
  if (written.length > 0) {
    console.info("✓ Generated icon files:")
    // biome-ignore lint/suspicious/useIterableCallbackReturn: false-positive
    written.forEach((line) => console.info(line))
  }
}
...
if (isWatchMode) {
  const REGISTRY_DIR = path.join(process.cwd(), "components")
