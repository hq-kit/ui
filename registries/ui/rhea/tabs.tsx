...
const tabsListVariants = tv({
  base: "rounded-2xl p-[3px] group-data-[orientation=horizontal]/tabs:h-8 data-[variant=line]:rounded-none group-data-[orientation=vertical]/tabs:p-1 group/tabs-list inline-flex w-fit items-center justify-center text-muted-foreground group-data-[orientation=vertical]/tabs:h-fit group-data-[orientation=vertical]/tabs:flex-col",
  variants: {
    variant: {
      default: "bg-muted",
...
const TabsTrigger = ({ className, ...props }: TabProps) => (
  <RACTab
    className={composeRenderProps(className, (className) =>
      cn(
        "gap-1.5 rounded-2xl border border-transparent! px-1.5 py-0.5 text-sm font-medium group-data-[orientation=vertical]/tabs:px-3 group-data-[orientation=vertical]/tabs:py-0.5 [&_svg:not([class*='size-'])]:size-4 group/tab relative isolate inline-flex h-[calc(100%-1px)] flex-1 items-center justify-center whitespace-nowrap text-foreground/60 transition-all hover:text-foreground focus-visible:border-ring focus-visible:outline-1 focus-visible:outline-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 data-disabled:pointer-events-none data-disabled:opacity-50 group-data-[orientation=vertical]/tabs:w-full group-data-[orientation=vertical]/tabs:justify-start dark:text-muted-foreground dark:hover:text-foreground [&_svg]:pointer-events-none [&_svg]:shrink-0",
...
const TabsContents = <T extends object>({ className, ...props }: TabPanelsProps<T>) => (
  <RACTabPanels
    {...props}
    className={cn(
      "text-sm relative h-(--tab-panel-height) flex-1 outline-none motion-safe:transition-[height]",
      className
    )}
  />
)
...
const TabsContent = ({ className, ...props }: TabPanelProps) => (
  <RACTabPanel
    className={composeRenderProps(className, (className) => cn("text-sm flex-1 outline-none", className))}
