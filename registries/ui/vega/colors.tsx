...
const ColorThumb = ({ className, ...props }: ColorThumbProps) => (
  <RACColorThumb
    className={composeRenderProps(className, (className) =>
      cn(
        "size-4 select-none rounded-full border border-primary bg-white shadow-sm ring-ring/50 transition-[color,box-shadow] [box-shadow:0_0_0_1px_var(--background),inset_0_0_0_1px_var(--foreground)] hover:ring-4 focus-visible:outline-hidden focus-visible:ring-4 disabled:opacity-50 group-data-[orientation=horizontal]/slider:top-1/2 group-data-[orientation=vertical]/slider:left-1/2",
...
const ColorSwatch = ({ className, ...props }: ColorSwatchProps) => (
  <RACColorSwatch
    className={composeRenderProps(className, (className) =>
      cn(
        "inset-ring-1 inset-ring-current/20 size-6 shrink-0 in-data-[slot=button-group]:rounded-md rounded-[min(var(--radius-md),8px)] rounded-md bg-muted [&_svg:not([class*='size-'])]:size-3",
        className
      )
    )}
