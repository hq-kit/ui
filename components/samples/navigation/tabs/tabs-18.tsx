    ...
    <div className='w-full max-w-md'>
      <Tabs className='flex-row' defaultSelectedKey='explore'>
        <TabsList className='h-full flex-col bg-background'>
          {tabs.map((tab) => (
            <TabsTrigger
              className='w-full data-selected:text-primary-foreground *:data-[slot=selected-indicator]:bg-primary dark:data-selected:text-primary-foreground dark:*:data-[slot=selected-indicator]:border-transparent dark:*:data-[slot=selected-indicator]:bg-primary'
              id={tab.value}
              key={tab.value}
            >
              {tab.name}
