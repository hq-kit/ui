...
const ListBox = <T extends object>({ className, ...props }: ListBoxProps<T>) => (
  <RACListBox
    ...
    className={composeRenderProps(className, (className) =>
      cn(
        "group/list-box flex size-full flex-col gap-1 overflow-hidden border bg-popover p-2 text-popover-foreground [--radius:0]",
...
const ListBoxItem = ({ children, className, ...props }: ListBoxItemProps) => {
  ...
  return (
    <RACListBoxItem
      className={composeRenderProps(className, (className) =>
        cn(
          "group/list-box-item relative flex cursor-default select-none items-center gap-2 in-data-[slot=dialog-content]:rounded-none rounded-none px-3 py-2 text-sm outline-hidden data-[disabled=true]:pointer-events-none data-selected:bg-muted data-selected:text-foreground data-[disabled=true]:opacity-50 [&_svg:not([class*='size-'])]:size-3.5 [&_svg]:pointer-events-none [&_svg]:shrink-0 data-selected:*:[svg]:text-foreground",
