...
const listStyles = tv({
  slots: {
    sectionVariants: 'col-span-full mt-2 grid grid-cols-[auto_1fr] text-sm',
    headerVariants: 'pointer-events-none col-span-full px-2 py-1 text-muted-foreground text-xs',
    itemVariants: [
...
const { sectionVariants, headerVariants, itemVariants } = listStyles()
      ...
      {(values) => (
        <>
          {values.allowsDragging && (
            <IconGripVertical
              className={cn('size-4 shrink-0 text-muted-foreground transition', values.isDragging && 'text-primary')}
            />
          )}
...
const ListBoxSection = <T extends object>({
  ...
  <RACListBoxSection className={sectionVariants({ className })}>
    {'title' in props && <Header className={headerVariants()}>{props.title}</Header>}
