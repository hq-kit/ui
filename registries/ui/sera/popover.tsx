...
const PopoverTitle = ({ className, ...props }: ComponentProps<"h2">) => (
  <div className={cn("text-xs font-semibold uppercase", className)} data-slot="popover-title" {...props} />
)

const PopoverDescription = ({ className, ...props }: ComponentProps<"p">) => (
  <p className={cn("text-muted-foreground text-sm leading-relaxed mt-0.5", className)} data-slot="popover-description" {...props} />
)
