...
const ColorThumb = ({ className, ...props }: ColorThumbProps) => (
  <RACColorThumb
    className={composeRenderProps(className, (className) =>
      cn(
        "h-4 w-6 select-none rounded-full bg-white not-dark:bg-clip-padding shadow-md ring-1 ring-black/10 transition-[color,box-shadow,background-color] [box-shadow:0_0_0_1px_var(--background),inset_0_0_0_1px_var(--foreground)] hover:ring-4 hover:ring-ring/30 focus-visible:outline-hidden focus-visible:ring-4 focus-visible:ring-ring/30 disabled:opacity-50 data-[orientation=vertical]:h-6 data-[orientation=vertical]:w-4 group-data-[orientation=horizontal]/slider:top-1/2 group-data-[orientation=vertical]/slider:left-1/2",
