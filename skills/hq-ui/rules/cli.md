...
- **If `registries` exists** — use the first key defined there (e.g. `@hq`, `@acme`, etc.)
- **If `registries` does NOT exist** — use `@hq` as the default. Do NOT modify `components.json` to add a registry.

IMPORTANT: Never add or modify the `registries` key in `components.json`. Just read it — if it's there, use it. If it's not, default to `@hq`.

## Step 3: Search the registry

The registry has two types of items:

- **`registry:ui`** — foundational components (button, tabs, dialog, etc.) installed to `src/components/ui/`
...
bunx --bun shadcn@latest search @hq -q "dialog"
```
...
bunx --bun shadcn@latest search @hq -q "dashboard"

# User asks for "login page" → search "login"
bunx --bun shadcn@latest search @hq -q "login"

# User asks for "settings" → search "settings"
bunx --bun shadcn@latest search @hq -q "settings"
```
...
```bash
# Install a component
bunx --bun shadcn@latest add @hq/dropdown-menu

# Install a block
bunx --bun shadcn@latest add @hq/sign-in-01
```

## Full example — installing a component

Suppose you need `<DropdownMenu>` but `src/components/ui/dropdown-menu.tsx` doesn't exist:

1. Check lockfile → `bun.lock` exists → use `bunx --bun`
2. Read `components.json` → registries key is `@hq` → use `@hq`
3. Search: `bunx --bun shadcn@latest search @hq -q "dropdown-menu"`
4. Find `dropdown-menu` with `"type": "registry:ui"` in results
5. Install: `bunx --bun shadcn@latest add @hq/dropdown-menu`
6. Import from `@/components/ui/dropdown-menu`
...
1. Check lockfile → `bun.lock` exists → use `bunx --bun`
2. Read `components.json` → registries key is `@hq` → use `@hq`
3. Search: `bunx --bun shadcn@latest search @hq -q "sign"`
4. Browse results for `"type": "registry:block"` items like `sign-in-01`
5. Install: `bunx --bun shadcn@latest add @hq/sign-in-01`
