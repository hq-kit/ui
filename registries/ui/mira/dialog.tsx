...
const DialogContent = ({
  ...
  <DialogOverlay isDismissable={isDismissable} {...props}>
    <RACModal
      className={cn(
        "bg-popover text-popover-foreground data-entering:animate-in data-exiting:animate-out data-exiting:fade-out-0 data-entering:fade-in-0 data-exiting:zoom-out-95 data-entering:zoom-in-95 ring-foreground/10 grid max-w-[calc(100%-2rem)] gap-4 rounded-xl p-4 text-xs/relaxed ring-1 duration-100 sm:max-w-sm fixed top-1/2 left-1/2 z-50 w-full -translate-x-1/2 -translate-y-1/2 outline-none",
        className
      )}
...
const DialogHeader = ({ className, ...props }: ComponentProps<"div">) => (
  <div className={cn("gap-1 flex flex-col", className)} data-slot="dialog-header" {...props} />
)
