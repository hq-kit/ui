...
function useMultibandVolume(mediaStream?: MediaStream | null, options: MultiBandVolumeOptions = {}) {
    ...
    return (
      ...
      >
        {volumeBands.map((volume, index) => {
          ...
          return <Bar heightPct={heightPct} isHighlighted={isHighlighted} key={index} state={state} />
        })}
      </div>
    )
  }
  ...
  return (
    <Card>
      ...
      <Card.Footer className="gap-2">
        ...
        <Button onClick={() => setState("listening")} size="sm" variant={state === "listening" ? "default" : "outline"}>
          Listening
        </Button>
