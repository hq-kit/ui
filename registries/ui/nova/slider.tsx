  ...
  >
    {({ state }) => (
      <>
        {children}
        <SliderTrack
          className="relative block grow select-none rounded-full bg-muted data-[orientation=horizontal]:h-1 data-[orientation=vertical]:h-full data-[orientation=horizontal]:w-full data-[orientation=vertical]:w-1"
          data-slot="slider-track"
        >
          <SliderFill className="select-none rounded-[inherit] bg-primary" data-slot="slider-range" />
          {state.values.map((_, i) => (
            <SliderThumb
              className="relative size-3 select-none rounded-full border border-ring bg-white ring-ring/50 transition-[color,box-shadow] after:absolute after:-inset-2 hover:ring-3 focus-visible:outline-hidden focus-visible:ring-3 active:ring-3 disabled:pointer-events-none disabled:opacity-50 group-data-[orientation=horizontal]/slider:top-1/2 group-data-[orientation=vertical]/slider:left-1/2"
