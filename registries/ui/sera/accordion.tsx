...
const Accordion = ({ className, ...props }: DisclosureGroupProps) => (
  <DisclosureGroup
    className={composeRenderProps(className, (className) => cn("flex w-full flex-col", className))}
    data-slot="accordion"
    {...props}
  />
)
...
const AccordionItem = ({ className, ...props }: DisclosureProps) => (
  <Disclosure
    className={composeRenderProps(className, (className) => cn("not-last:border-b", className))}
...
const AccordionContent = ({ className, children, ...props }: DisclosurePanelProps) => {
  return (
    <DisclosurePanel
      className="text-sm h-(--disclosure-panel-height) overflow-hidden transition-[height] duration-200"
