    ...
    <div className='w-full max-w-md'>
      <Tabs className='gap-4' defaultSelectedKey='explore'>
        <TabsList className='bg-background'>
          {tabs.map((tab) => (
            <TabsTrigger
              className='data-selected:text-primary *:data-[slot=selected-indicator]:bg-primary/20 *:data-[slot=selected-indicator]:shadow-none dark:data-selected:text-primary dark:*:data-[slot=selected-indicator]:border-transparent dark:*:data-[slot=selected-indicator]:bg-primary/20'
              id={tab.value}
              key={tab.value}
            >
              {tab.name}
