      ...
      <SwitchButton data-slot="switch">
        {composeRenderProps(children, (children) => (
          <div className={cn(fieldVariants({ orientation: "horizontal" }))}>
            <div
              className={cn(
                "peer group/switch relative inline-flex shrink-0 items-center rounded-2xl border-2 not-group-data-selected/field:border-transparent not-group-data-selected/field:bg-input/90 outline-none transition-all after:absolute focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/30 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 data-[size=default]:h-5 data-[size=sm]:h-4 data-[size=default]:w-8 data-[size=sm]:w-6 group-data-disabled/field:cursor-not-allowed group-data-selected/field:border-primary group-data-selected/field:bg-primary group-data-disabled/field:opacity-50 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40",
                className
              )}
              data-size={size}
              data-slot="switch"
            ...
            >
              <span
                className="pointer-events-none block not-group-data-selected/field:translate-x-0 rounded-2xl bg-background not-dark:bg-clip-padding shadow-sm ring-0 transition-transform group-data-[size=default]/switch:size-4 group-data-[size=sm]/switch:size-3 group-data-selected/field:translate-x-[calc(100%-4px)] dark:not-group-data-selected/field:bg-foreground dark:group-data-selected/field:bg-primary-foreground"
