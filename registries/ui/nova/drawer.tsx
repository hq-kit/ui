        ...
        >
          {({ state }) => (
            <Modal>
              ...
              >
                {notch && side === "bottom" && <div className="notch bg-muted mx-auto group-data-[direction=top]/drawer-content:mb-4 group-data-[direction=bottom]/drawer-content:mt-4 h-1 w-25 shrink-0 rounded-full shrink-0 touch-pan-y" />}
                {children as ReactNode}
                {notch && side === "top" && <div className="notch bg-muted mx-auto group-data-[direction=top]/drawer-content:mb-4 group-data-[direction=bottom]/drawer-content:mt-4 h-1 w-25 shrink-0 rounded-full mx-auto shrink-0 touch-pan-y" />}
              </DrawerRoot>
            </Modal>
          )}
        </DrawerOverlay>
...
const DrawerHeader = ({ className, ...props }: ComponentPropsWithRef<"div">) => {
  return <div className={cn("gap-0.5 p-4 group-data-[direction=bottom]/drawer-content:text-center group-data-[direction=top]/drawer-content:text-center md:gap-0.5 md:text-left flex flex-col", className)} slot="header" {...props} />
}

const DrawerTitle = ({ className, ...props }: RACHeadingProps) => (
  <Heading className={cn("text-foreground text-base font-medium", className)} slot="title" {...props} />
)
