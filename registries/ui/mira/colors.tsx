...
const ColorArea = ({ className, ...props }: ColorAreaProps) => (
  <RACColorArea
    className={composeRenderProps(className, (className) =>
      cn("bg-muted rounded-md size-48 shrink-0 shadow-md data-disabled:opacity-50 data-disabled:grayscale-50", className)
    )}
    {...props}
  >
    <ColorThumb />
...
const ColorThumb = ({ className, ...props }: ColorThumbProps) => (
  <RACColorThumb
    className={composeRenderProps(className, (className) =>
      cn(
        "border-ring ring-ring/30 relative size-3 rounded-md border bg-white transition-[color,box-shadow] after:absolute after:-inset-2 hover:ring-2 focus-visible:ring-2 focus-visible:outline-hidden active:ring-2 select-none [box-shadow:0_0_0_1px_var(--background),inset_0_0_0_1px_var(--foreground)] disabled:opacity-50 group-data-[orientation=horizontal]/slider:top-1/2 group-data-[orientation=vertical]/slider:left-1/2",
...
const ColorSwatchPickerItem = ({ className, children, ...props }: ColorSwatchPickerItemProps) => (
  <RACColorSwatchPickerItem
    className={composeRenderProps(className, (className) =>
      cn(
        "bg-muted rounded-md relative outline-hidden",
...
const ColorSwatch = ({ className, ...props }: ColorSwatchProps) => (
  <RACColorSwatch
    className={composeRenderProps(className, (className) =>
      cn("size-5 rounded-sm [&_svg:not([class*='size-'])]:size-2.5 bg-muted rounded-md inset-ring-1 inset-ring-current/20 shrink-0", className)
    )}
