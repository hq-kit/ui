...
const ColorThumb = ({ className, ...props }: ColorThumbProps) => (
  <RACColorThumb
    className={composeRenderProps(className, (className) =>
      cn(
        "relative size-3 select-none rounded-md border border-ring bg-white ring-ring/30 transition-[color,box-shadow] [box-shadow:0_0_0_1px_var(--background),inset_0_0_0_1px_var(--foreground)] after:absolute after:-inset-2 hover:ring-2 focus-visible:outline-hidden focus-visible:ring-2 active:ring-2 disabled:opacity-50 group-data-[orientation=horizontal]/slider:top-1/2 group-data-[orientation=vertical]/slider:left-1/2",
...
const ColorSwatch = ({ className, ...props }: ColorSwatchProps) => (
  <RACColorSwatch
    className={composeRenderProps(className, (className) =>
      cn(
        "inset-ring-1 inset-ring-current/20 size-5 shrink-0 rounded-md rounded-sm bg-muted [&_svg:not([class*='size-'])]:size-2.5",
        className
      )
    )}
