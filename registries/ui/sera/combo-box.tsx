...
const ComboBoxInput = (props: InputProps) => (
  <span className="[&_svg:not([class*='size-'])]:size-3.5 relative isolate block [&_input]:pr-8" data-slot="control" slot="control">
...
const ComboBoxContent = <T extends object>({
  ...
  <Popover
    className={cn(
      "bg-popover text-popover-foreground data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 ring-foreground/10 *:data-[slot=input-group]:bg-transparent *:data-[slot=input-group]:border-b-input max-h-72 min-w-36 overflow-hidden rounded-none shadow-md ring-1 duration-100 *:data-[slot=input-group]:m-1.5 *:data-[slot=input-group]:mb-0 *:data-[slot=input-group]:h-8 *:data-[slot=input-group]:border-transparent *:data-[slot=input-group]:focus-within:border-transparent *:data-[slot=input-group]:px-2.5 data-[placement=start]:slide-in-from-right-2 data-[placement=end]:slide-in-from-left-2 group/combobox-content relative min-w-(--trigger-width)",
      className
    )}
    offset={offset}
    placement={placement}
...
const ComboBoxSeparator = ({ className, ...props }: SeparatorProps) => (
  <Separator className={cn("bg-border/50 -mx-1.5 my-1.5 h-px", className)} data-slot="select-separator" {...props} />
)
