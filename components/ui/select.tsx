...
const SelectValue = <T extends object>({ className, children, ...props }: SelectValueProps<T>) => (
  <RACSelectValue className={cn("cn-select-value cn-select-value-aria", className)} data-slot="select-value" {...props}>
  ...
  return (
    ...
    >
      {composeRenderProps(children, (children, { isSelected }) => (
        <>
          <span className="cn-select-item-text shrink-0 items-center whitespace-nowrap">{children}</span>
...
const SelectEmpty = ({ className, ...props }: ComponentProps<"div">) => (
  <div className={cn("cn-select-empty-aria", className)} data-slot="select-empty" {...props} />
)
