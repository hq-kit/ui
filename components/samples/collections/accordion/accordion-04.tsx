  ...
  return (
    <Accordion className='w-full' defaultExpandedKeys='item-1'>
      {items.map((item, index) => (
        <AccordionItem
          className='in-data-[expanded=true]:border-amber-600 not-last:in-data-[expanded=true]:border-b-2 dark:in-data-[expanded=true]:border-amber-400'
          id={`item-${index + 1}`}
          key={index}
        >
          <AccordionTrigger className='in-data-[expanded=true]:text-amber-600 dark:in-data-[expanded=true]:text-amber-400 in-data-[expanded=true]:*:[svg]:text-amber-600 dark:in-data-[expanded=true]:*:[svg]:text-amber-400'>
            {item.title}
          </AccordionTrigger>
