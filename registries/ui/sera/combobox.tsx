...
const ComboboxContent = <T extends object>({
  ...
  <Popover
    className={cn(
      "bg-popover text-popover-foreground data-entering:animate-in data-exiting:animate-out data-exiting:fade-out-0 data-entering:fade-in-0 data-exiting:zoom-out-95 data-entering:zoom-in-95 data-[placement=bottom]:slide-in-from-top-2 data-[placement=left]:slide-in-from-right-2 data-[placement=right]:slide-in-from-left-2 data-[placement=top]:slide-in-from-bottom-2 ring-foreground/10 *:data-[slot=input-group]:bg-transparent max-h-72 min-w-36 overflow-hidden rounded-none shadow-md ring-1 duration-100 *:data-[slot=input-group]:m-1.5 *:data-[slot=input-group]:mb-0 *:data-[slot=input-group]:h-8 *:data-[slot=input-group]:border-transparent *:data-[slot=input-group]:focus-within:border-transparent *:data-[slot=input-group]:px-2.5 relative isolate z-50 w-(--trigger-width) origin-(--trigger-anchor-point)",
      className
    )}
    offset={offset}
    placement={placement}
...
const ComboboxGroup = <T extends object>({ title, ...props }: ListBoxSectionProps<T> & { title?: string }) => (
  <ListBoxSection className={cn(props.className)} data-slot="combobox-group" {...props}>
    {title && (
      <Header className="px-3 py-2 text-xs uppercase tracking-wider font-semibold text-muted-foreground" data-slot="combobox-label">
        {title}
      </Header>
    )}
...
const ComboboxSeparator = ({ className, ...props }: SeparatorProps) => (
  <Separator className={cn("bg-border/50 -mx-1.5 my-1.5 h-px", className)} data-slot="combobox-separator" {...props} />
)
