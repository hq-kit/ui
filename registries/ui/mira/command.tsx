...
const CommandList = <T extends object>({ className, ...props }: MenuProps<T>) => (
  <Menu
    {...props}
    className={cn("no-scrollbar max-h-72 flex-1 scroll-py-1 overflow-y-auto overflow-x-hidden outline-none", className)}
...
const CommandSeparator = ({ className, ...props }: SeparatorProps) => (
  <Separator className={cn("-mx-1 my-1 h-px bg-border/50", className)} data-slot="command-separator" {...props} />
)
...
const CommandItem = <T extends object>({ className, children, textValue, ...props }: MenuItemProps<T>) => (
  <MenuItem
    {...props}
    className={cn(
      "group/command-item relative flex min-h-7 cursor-default select-none items-center gap-2 in-data-[slot=dialog-content]:rounded-md rounded-md px-2.5 py-1.5 text-xs/relaxed outline-hidden data-[disabled=true]:pointer-events-none data-focused:bg-muted data-selected:bg-muted data-focused:text-foreground data-selected:text-foreground data-[disabled=true]:opacity-50 [&_svg:not([class*='size-'])]:size-3.5 [&_svg]:pointer-events-none [&_svg]:shrink-0 data-focused:*:[svg]:text-foreground data-selected:*:[svg]:text-foreground",
      className
    )}
...
const CommandShortcut = ({ className, ...props }: ComponentProps<"span">) => (
  <span
    className={cn(
      "ml-auto text-[0.625rem] text-muted-foreground tracking-widest group-data-focused/command-item:text-foreground group-data-selected/command-item:text-foreground",
      className
    )}
