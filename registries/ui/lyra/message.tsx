...
const Message = ({ className, align = "start", ...props }: ComponentProps<"div"> & { align?: "start" | "end" }) => (
  <div
    className={cn("text-xs gap-1.5 group/message relative flex w-full min-w-0 data-[align=end]:flex-row-reverse", className)}
    data-align={align}
    data-slot="message"
    {...props}
  />
...
const MessageHeader = ({ className, ...props }: ComponentProps<"div">) => (
  <div
    className={cn("text-xs font-medium text-muted-foreground px-2.5 group-has-data-[variant=ghost]/message:px-0 flex min-w-0 max-w-full items-center", className)}
...
const MessageFooter = ({ className, ...props }: ComponentProps<"div">) => (
  <div
    className={cn(
      "text-xs font-medium text-muted-foreground px-2.5 group-has-data-[variant=ghost]/message:px-0 flex min-w-0 max-w-full items-center group-data-[align=end]/message:justify-end",
      className
    )}
