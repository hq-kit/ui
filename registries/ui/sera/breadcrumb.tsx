  ...
  return (
    <BreadcrumbsProvider value={{ separator }}>
      <RACBreadcrumbs
        ...
        {...props}
        className={cn("text-muted-foreground gap-1.5 text-xs uppercase tracking-wide sm:gap-2.5 wrap-break-word flex flex-wrap items-center", className)}
      />
    </BreadcrumbsProvider>
  )
}
  ...
  return (
    <RACBreadcrumb
      className={composeRenderProps(className, (className, { isCurrent }) =>
        cn(
          "gap-1.5 inline-flex items-center [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
