...
const SelectGroup = <T extends object>({ title, children, ...props }: ListBoxSectionProps<T> & { title?: string }) => (
  <ListBoxSectionPrimitive className={cn("scroll-my-1.5 p-1.5", props.className)} data-slot="select-group" {...props}>
    {title && (
      <HeaderPrimitive className="px-3 py-2 text-xs uppercase tracking-wider font-semibold text-muted-foreground" data-slot="select-label">
        {title}
      </HeaderPrimitive>
    )}
