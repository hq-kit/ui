  ...
  return (
    <div className='w-full max-w-md'>
      <Tabs className='gap-4' defaultSelectedKey='explore'>
        <TabsList className='gap-1 border bg-background p-1'>
          {tabs.map((tab) => (
            <TabsTrigger
              className='data-selected:text-primary-foreground *:data-[slot=selected-indicator]:bg-primary dark:data-selected:text-primary-foreground dark:*:data-[slot=selected-indicator]:border-transparent dark:*:data-[slot=selected-indicator]:bg-primary'
              id={tab.value}
              key={tab.value}
            >
              {tab.name}
