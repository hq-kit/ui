...
const fieldVariants = tv({
  base: "group/field flex w-full gap-2 has-data-invalid:text-destructive data-[invalid=true]:text-destructive data-invalid:text-destructive hover:not-data-disabled:not-in-data-disabled:**:[[slot=control]]:border-ring!",
  variants: {
    orientation: {
...
const Label = ({ className, htmlFor, slot, ...props }: LabelProps) => {
  const label = (
    <RACLabel
      className={cn(
        "in-data-disabled:pointer-events-none flex in-data-disabled:cursor-not-allowed select-none items-center gap-2 font-medium text-xs/relaxed leading-none peer-disabled:opacity-50 group-data-[disabled=true]:opacity-50",
        "group/field-label peer/field-label flex w-fit gap-2 leading-snug has-[>[data-slot=field]]:rounded-md has-[>[data-slot=field]]:border has-group-data-selected/field:bg-primary/5 *:data-[slot=field]:p-2 group-data-[disabled=true]/field:opacity-50 dark:has-group-data-selected/field:bg-primary/10",
  ...
  return (
    <RACFieldError
      className={composeRenderProps(props.className, (className) =>
        cn("font-normal text-destructive text-xs/relaxed", className)
      )}
