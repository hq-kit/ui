...
const Root = ({ className, ...props }: TableProps) => (
  <RACTable
    className={composeRenderProps(className, (className) => cn("w-full caption-bottom text-xs", className))}
    {...props}
  />
)
...
const TableColumn = ({ isResizable = false, className, ...props }: ColumnProps & { isResizable?: boolean }) => {
  return (
    <Column
      ...
      className={composeRenderProps(className, (className) =>
        cn(
          "relative h-10 whitespace-nowrap px-2 text-left align-middle font-medium text-foreground [&:has([role=checkbox])]:pr-0",
...
const TableCell = ({ className, ref, ...props }: TableCellProps) => {
  ...
  return (
    <Cell
      ...
      className={composeRenderProps(className, (className) =>
        cn(
          "whitespace-nowrap p-2 align-middle outline outline-transparent [&:has([role=checkbox])]:pr-0",
