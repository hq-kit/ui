...
const ComboboxItem = <T extends object>({ className, children, ...props }: ListBoxItemProps<T>) => {
  ...
  return (
    <ListBoxItem
      className={cn(
        "relative flex w-full cursor-default select-none items-center gap-2.5 rounded-2xl py-2 pr-8 pl-3 font-medium text-sm outline-hidden data-disabled:pointer-events-none data-focused:bg-accent data-highlighted:bg-accent data-focused:text-accent-foreground data-highlighted:text-accent-foreground data-disabled:opacity-50 not-data-[variant=destructive]:data-focused:**:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className
      )}
...
const ComboboxGroup = <T extends object>({ title, ...props }: ListBoxSectionProps<T> & { title?: string }) => (
  <ListBoxSection className={cn(props.className)} data-slot="combobox-group" {...props}>
    {title && (
      <Header className="px-3 py-2.5 text-muted-foreground text-xs" data-slot="combobox-label">
        {title}
      </Header>
    )}
...
const ComboboxSeparator = ({ className, ...props }: SeparatorProps) => (
  <Separator className={cn("-mx-1.5 my-1.5 h-px bg-border", className)} data-slot="combobox-separator" {...props} />
)
